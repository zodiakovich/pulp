-- generations: ensure table + columns, API grants, RLS (Clerk JWT sub = user_id)
-- Run via `supabase db push` or paste into Supabase SQL Editor if migrations were never applied.
-- PostgREST 404 on /rest/v1/generations usually means the relation is missing from public schema.
...
-- ---------------------------------------------------------------------------
-- Indexes (optional but helps history / explore)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Grants: expose table to PostgREST roles
-- ---------------------------------------------------------------------------
...
-- SELECT: own rows (JWT sub or Supabase uid) OR public gallery rows.
-- Clerk: configure JWT template "supabase" so `sub` equals Clerk user id stored in generations.user_id.
